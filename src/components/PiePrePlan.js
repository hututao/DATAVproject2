...
const transPreplan = (data) => {
    ...
    data.forEach((item) => {
        ...
        const preplan = item.preffered_premium_plan;
        ...
        if (preplan) {
            result[key][preplan] += 1;
        }
    });

    const convertToEChartsFormat = (data) => {
        return Object.keys(data).map(preplan => ({
            value: data[preplan],
            name: `${preplan}`
        }));
    };
...
const PiePreplan = ({ data, highlightedCategory }) => {
...
export { PiePreplan, transPreplan };
